...
test.describe('PLP - Test cases', () => {
  ...
  test('TC - 01 User can search for a product and see relevant results', async ({ page }) => { 
    ...
    const firstProductDetails = await productsPage.getFirstProductDetails();
    await expect(firstProductDetails.name).toBe(expectedProducts[0].name);
  });

    test('TC - 02 Promo big banner is visible', async ({ page }) => { 
    await globalPage.navigateToProducts();
    expect (page.getByRole('img', { name: 'Website for practice' })).toBeVisible()
  });
